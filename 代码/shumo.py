...
class Individual:
    ...
    def cal_object_value(self):
        ...
        """
        self.M[0], self.M[1], self.M[2], self.M[3] = problems.Func(D=self.Dm, T=self.Timelist,
                                                                   Num=self.travelnum,
                                                                   Dis=self.travelDis,
                                                                   w=Food)
    ...
    def plan(self):
        ...
        self.money += (self.Timelist[0][0]/24)*sleepmoney
...
def judge(X,state):
    ...
    for i in range(len(travellist)):
        ...
        if(Food[sight][0]==1):
            flag=True
            for k in range(travellist):
                if b[travellist[k]].index(1) == city:
                    remT=travellist[k]
                    travellist.remove(remT)
            continue
    ...
    money +=(Timelist[0][0]/24)*sleepmoney
    if(money > 5000):
        flag=False
        return False
...
for i in range(maxepoch):
    ...
    #住宿费
    money +=days*sleepmoney
    if (I.Timelist[0][0] / 24 >= 14):
        flag = False
